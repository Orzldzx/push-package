...
class Backing(object):
    ...
    def changeService(self, services_options, handle_service_names):
        ...
        for handle_type in [ 'reload', 'restart' ]:
            if not handle_service_names[handle_type]: continue
            try:
                ...
                for handle_service_name in handle_service_names[handle_type]:
                    ...
                    if '.' in handle_service_name:
                        ...
                        host = services_options[_srv[0]][int(_srv[1])-1]['lip']
...
#                   tnum = 'srv'
#                   cmd = '/bin/bash /svndata/svr/ServerAdm.sh'
...
if __name__ == '__main__':
    ...
    # 加载服务列表
    services_options = load_yaml('../conf/tex-push.yml')
    handle_service_names = load_yaml('../conf/tex-service.yml')
    # 加载回滚主机列表
